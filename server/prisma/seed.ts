...
async function main() {
  ...
  const addr1 = await prisma.address.create({
    data: {
      ...
      streetAddress: 'Flat 204, Rosewood Apartments, Koramangala 4th Block',
  ...
  const addr2 = await prisma.address.create({
    data: {
      ...
      receiverName: 'Aravind K.',
      receiverPhone: '+919876543210',
      streetAddress: 'Aether Technologies, Sector 7, HSR Layout',
  ...
  const addr3 = await prisma.address.create({
    data: {
      ...
      streetAddress: '12, 10th Cross, Indiranagar',
      postalCode: '560038',
      city: 'Bangalore',
      latitude: 12.9720,
      longitude: 77.5950,
  ...
  const rider1 = await prisma.rider.create({
    data: {
      ...
      vehiclePlateNumber: 'KA-01-EQ-9999',
      isOnline: true,
      isApproved: true,
