...
import { Search, Bell, History, Trash2, X, ShoppingBag, Home, Grid, User } from 'lucide-react';
...
export const CustomerHeader: React.FC<CustomerHeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-100 bg-bg-secondary/90 backdrop-blur-md border-b border-border-primary w-full">
      {/* Mobile Layout */}
      <div className="md:hidden flex flex-col gap-3 px-4 py-2">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <img 
              src="/Aether_Mart_Logo-preview.png"
              alt="Aether Mart"
              onClick={() => navigate('/c/home')}
              className="h-20 w-auto cursor-pointer select-none"
            />
      ...
      <div className="hidden md:flex items-center justify-between gap-4 px-4 py-2">
        {/* Left Side: Logo → Location → Home → Cart */}
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => openDrawer('CART')}
            className="px-3 py-2 rounded-xl border border-brand-emerald/30 bg-brand-emerald/10 text-brand-emerald hover:bg-brand-emerald/20 transition-all relative flex items-center gap-1.5 cursor-pointer font-bold text-xs font-heading"
            aria-label="Open cart drawer"
          >
            <ShoppingBag className="h-4 w-4" />
            <span>Cart</span>
        ...
        {/* Right Side: Categories → Orders → Notification → Account */}
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate('/c/category/all')}
            className={cn(
              "flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer",
          ...
          <button
            onClick={() => navigate('/c/profile?tab=orders')}
            className={cn(
              "flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer",
              location.pathname === '/c/profile' && new URLSearchParams(location.search).get('tab') === 'orders'
                ? "bg-brand-emerald/10 text-brand-emerald"
                : "text-text-secondary hover:text-text-primary hover:bg-bg-tertiary"
            )}
          >
          ...
          <button
            onClick={() => navigate('/c/profile?tab=profile')}
            className={cn(
              "flex items-center gap-2 px-3 py-2 p-2 rounded-xl border border-border-primary text-xs font-bold transition-all cursor-pointer",
              location.pathname === '/c/profile' && (new URLSearchParams(location.search).get('tab') === 'profile' || !new URLSearchParams(location.search).get('tab'))
                ? "bg-brand-emerald/10 text-brand-emerald"
                : "text-text-secondary hover:text-text-primary hover:bg-bg-tertiary"
            )}
          >
